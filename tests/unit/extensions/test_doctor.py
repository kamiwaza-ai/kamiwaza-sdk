...
class TestDoctorExtensionChecks:
    ...
    def test_extension_context_rejects_undeclared_compose_capability(
        ...
        compatibility = next(
            result for result in results if result.name == "CLI version compatibility"
        )
    ...
    def test_python_runtime_lib_found(self, tmp_path):
        ...
        # (`>=0.6,<0.7` for Python at time of writing) so generated apps
        # can rely on the runtime relocation helpers.
        req_file.write_text("kamiwaza-extensions-lib>=0.6.0,<0.7\nfastapi\n")
        checker = DoctorChecker(config_dir=tmp_path / ".kamiwaza")
    ...
    def test_ts_runtime_lib_found(self, tmp_path):
        ...
        pkg_file.write_text(
            json.dumps({"dependencies": {"@kamiwaza-ai/extensions-lib": "^0.6.0"}})
        )
        checker = DoctorChecker(config_dir=tmp_path / ".kamiwaza")
