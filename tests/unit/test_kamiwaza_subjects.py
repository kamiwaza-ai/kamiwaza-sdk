...
def test_upsert_puts_to_server_with_attributes(mock_client) -> None:
    ...
    mock_client.expect(
        ...
        "/authz/subjects/alice",
        {
            "id": "kc-alice-uuid",
            "username": "alice",
            "attributes": {"access_tier": "S", "country": "GBR"},
            "grants": [],
    ...
    result = SubjectsAPI(client=mock_client).upsert(
        "alice", attributes={"access_tier": "S", "country": "GBR"}
    )

    assert isinstance(result, Subject)
    assert result.id == "kc-alice-uuid"
    assert result.username == "alice"
    assert result.attributes["access_tier"] == "S"
...
def test_upsert_forwards_password_in_body(mock_client) -> None:
    ...
    mock_client.expect(
        ...
        "/authz/subjects/bob",
        {
            "id": "kc-bob-uuid",
            "username": "bob",
            "attributes": {"access_tier": "U"},
            "grants": [],
        },
    )

    SubjectsAPI(client=mock_client).upsert(
        "bob", attributes={"access_tier": "U"}, password="initial-pw"
    )

    body = mock_client.calls[0][2].get("json", {})
    assert body == {"attributes": {"access_tier": "U"}, "password": "initial-pw"}
