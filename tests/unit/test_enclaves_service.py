...
from kamiwaza_sdk.schemas.enclaves import (
    ConnectorCreate,
    ConnectorUpdate,
...
def _document_record(
    document_id: UUID | str | None = None, *, source_id: UUID | str | None = None
) -> dict:
...
def test_create_connector_preserves_explicit_none_fields(dummy_client):
    ...
    assert client.calls[0][2]["json"]["default_marking"] is None
...
def test_update_connector_preserves_explicit_none_fields(dummy_client):
    ...
    service.connectors.update(
        connector_id,
        ConnectorUpdate(description=None, default_marking=None),
    )
...
def test_trigger_ingest_posts_request(dummy_client):
    connector_id = uuid4()
    responses = {
        ("post", f"/enclaves/connectors/{connector_id}/trigger_ingest"): {
            "status": "queued"
        }
    }
    client = dummy_client(responses)
...
def test_create_document_preserves_explicit_none_fields(dummy_client):
    ...
    assert "marking_text" in client.calls[0][2]["json"]
    assert client.calls[0][2]["json"]["marking_text"] is None


def test_list_documents_sets_marking_level_header(dummy_client):
    source_id = uuid4()
    ...
    assert kwargs["headers"]["X-User-Marking-Level"] == "PUBLIC"


def test_list_documents_preserves_caller_supplied_marking_level_header(dummy_client):
    source_id = uuid4()
    document = _document_record(source_id=source_id)
    responses = {
        ("get", "/enclaves/documents/"): {
    ...
    service.documents.list(
        source_id,
        headers={"X-User-Marking-Level": "CONFIDENTIAL"},
        marking_level="PUBLIC",
    )

    assert client.calls[0][2]["headers"]["X-User-Marking-Level"] == "CONFIDENTIAL"
...
def test_get_document_passes_source_id_and_header(dummy_client):
    ...
    result = service.documents.get(
        document_id, source_id=source_id, marking_level="PUBLIC"
    )
...
def test_connector_response_tolerates_missing_optional_fields(dummy_client):
    ...
    assert result.default_marking is None
...
def test_connector_api_errors_propagate(dummy_client):
    client = dummy_client({})
    client.get = lambda path, **kwargs: (_ for _ in ()).throw(
        APIError("boom", status_code=404, response_text="")
    )  # noqa: E731
    service = EnclavesService(client)
