"""Receiver-assigned onboarding access tier reaches a mesh gate.

The receiver approves a self-service guest with ``access_tier=standard`` and a
viewer grant on an access-tier dataset. The requester claims the receiver-realm
credential, presents it during mesh retrieval, and receives exactly the basic
and standard rows plus the gate audit footer.

``ACCESS_TIER_DATASET_PATH`` points to the receiver-visible CSV written from
``access_tier_records.json`` by ``tests.integration._gate_fixture``. The test
creates its own dataset catalog row and gate binding. It never removes the
shared fixture.
"""
...
def _provision_pair(state: dict[str, Any], live_peer_base_url: str) -> None:
    ...
    name = f"eng10096-access-tier-{uuid.uuid4().hex[:10]}"
    psk = uuid.uuid4().hex
...
def _approve_and_claim(state: dict[str, Any], requester: KamiwazaClient) -> str:
    status = _self_request_onboarding(
        requester,
        state["initiator_id"],
        "ENG-10096 receiver-assigned access-tier gate proof.",
    )
    ...
    approved = _obj(
        ...
        json={
            "attributes": {"access_tier": "standard"},
            "relations": [
                {
                    "relation": "viewer",
...
def test_receiver_assigned_access_tier_claim_reaches_dataset_gate_over_mesh(
    ...
    assert path["assigned_attributes"] == {"access_tier": "standard"}

    credential = path["credential"]
    claims = _decode_jwt_payload(credential)
    assert claims.get("access_tier") == "standard", claims
    ...
    expected = [
        row for row in mc.records() if row["required_tier"] in {"basic", "standard"}
    ]
    assert sorted(rows, key=lambda row: row["id"]) == expected
    mc.assert_persona_result("standard", rows, gate_audits)
