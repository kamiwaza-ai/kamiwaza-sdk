...
DATASET_DIR = "/app/tmp"
DATASET_PATH = f"{DATASET_DIR}/eng10050-mini-access_tier.csv"
PUBLISH_ATTEMPTS = 3
...
def stage_index(
    ...
    from tests.integration import _mini_access_tier as mc  # noqa: PLC0415

    mc.write_dataset_file(STAGE_DIR / "mini_access_tier.csv")
    return STAGE_DIR
...
def _publish_item(argv: list[str], target: RayPodTarget, item: Path, leaf: str) -> None:
    ...
    destination = (
        DATASET_PATH if item.name == "mini_access_tier.csv" else f"{leaf}/{item.name}"
    )
    raw = item.read_bytes()
...
def publish(argv: list[str], directory: Path) -> str:
    ...
    for item in items:
        destinations = targets if item.name == "mini_access_tier.csv" else (head,)
...
def verify(
    ...
    *,
    ...
    dataset_path = dataset or STAGE_DIR / "mini_access_tier.csv"
...
def _fixture_environment(network_index_url: str | None = None) -> dict[str, str]:
    ...
    values = {
        ...
        "MINI_ACCESS_TIER_DATASET_PATH": DATASET_PATH,
    }
