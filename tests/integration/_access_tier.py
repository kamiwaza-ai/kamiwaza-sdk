"""Shared fixtures and helpers for access-tier gate live tests.

Single-cluster and federated layers prove wheel installation, file-backed data,
gate binding, and server-side invocation with deterministic basic, standard,
and advanced access tiers.
"""
...
# persona access tier -> (included, redacted, allowed record tiers)
...
def _already_installed(kz: Any) -> bool:
    """Return whether the expected access-tier gate package is installed.
...
def install_gate_package(kz: Any, wheel_dir: str, index_url: str) -> None:
    """Ensure the expected access-tier gate is installed and discoverable.
...
class _NoCacheTokenStore:
    """No-op token store so each persona authenticates without disk state."""
...
def shared_realm_token(
    ...
    *,
    ...
    JWKS). The token also carries ``access_tier`` and an explicit
    ``tenant_id=__default__`` claim for the gate.
    """
...
def create_file_dataset(kz: Any, name: str, file_path: str) -> str:
    """Create a file-backed dataset and bind the access-tier gate."""
...
def retrieve_through_gate(
    ...
    ``null`` because the volume of withheld material can disclose policy state.
    Summing them here raised
...
def _assert_exact_fixture_rows(
    ...
    expected_rows = sorted(
        (
            ...
            if str(record.get("required_tier", "")).lower() in allowed
        ),
...
def assert_persona_result(
    ...
    # zero leakage: no row above the caller's access tier survives
    leaked = [r for r in rows if str(r.get("required_tier", "")).lower() not in allowed]
    assert not leaked, f"{access_tier} caller leaked higher-tier rows: {leaked}"
