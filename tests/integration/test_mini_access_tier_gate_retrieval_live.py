...
def _prereqs() -> tuple[str, str, str]:
    ...
    if not dataset_path:
        pytest.skip(
            "MINI_ACCESS_TIER_DATASET_PATH not set — place the 5-row fixture "
...
def gated_dataset(_prereqs, _admin) -> Iterator[str]:
    ...
    urn = mc.create_file_dataset(kz, _unique("mini-access_tier"), dataset_path)
...
def test_persona_sees_exact_post_gate_counts(
    ...
    """Each access_tier persona retrieves exactly its allowed rows — known answer."""
