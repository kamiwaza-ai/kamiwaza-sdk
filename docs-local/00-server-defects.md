...
- **Workroom enter binding unavailable.** `tests/integration/test_seeding_live.py::test_workroom_create_enter_scopes_a_client` still skips. A direct create/enter/delete probe returned `503` with `binding_unavailable` for `workrooms.enter`; cleanup delete succeeded.
