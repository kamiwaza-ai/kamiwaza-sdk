...
- 2026-09-16 (ENG-12320): `test_catalog_large_object_sse_retrieval` is now `test_catalog_sse_retrieval_emits_terminal_event`. It streams the small object and requires `chunk` events, a terminal `complete` event and a COMPLETED job. On 1.2.1 the stream returns HTTP 200 with no events and the job ends FAILED, so the test fails (ENG-12300).
