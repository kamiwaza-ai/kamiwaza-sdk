...
- 2026-09-16 (ENG-12320): `test_catalog_large_object_sse_retrieval` is now `test_catalog_sse_retrieval_emits_terminal_event`. It streams the small object and requires `chunk` events, a terminal `complete` event and a COMPLETED job. On the Azure 1.2.1 evidence instance the stream returned HTTP 200 with no events and the job ended FAILED, so the test fails there; the cause is not yet established (ENG-12300).
