...
"""Client service for cluster-wide connectors.

Registers the connector *app* (admin-scoped, one per type per cluster) — either a
catalog connector by ``connector_type`` + config, or an out-of-core connector by
manifest + endpoint. The client is connector-agnostic: ``connector_type`` is an
open string the platform resolves against the catalog, and ``config`` is an opaque
