...
def _run_authorized_discovery(context: RunContext) -> None:
    body = _remote_models(context, "access-basic")
...
def _run_unauthorized(context: RunContext) -> None:
    try:
        _remote_chat(context, "access-standard")
    except Exception as exc:
        if getattr(exc, "status_code", None) in {401, 403}:
            return
        raise
    raise AssertionError("model without a receiver invoke grant was callable")
...
def _remote_models(context: RunContext, username: str) -> Any:
    ...
    try:
        ...
        if username == "access-standard" and getattr(exc, "status_code", None) in {
            401,
            403,
        }:
            return exc
