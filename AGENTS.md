...
- **`kamiwaza-federation` CLI (ENG-8213):** an operator/test utility (`kamiwaza_sdk.seeding.federation`; console script `kamiwaza-federation`) that productizes shared_idp stand-up + ReBAC access seeding that previously lived only in the L3 test fixtures (`tests/integration/_mini_access_tier.py`). Groups: `access` (ReBAC grants), `fed` (pair / status / allow-user / unpair), `dataset` / `gate` / `attr` (gated-retrieval setup), and `idp` (**DEV/TEST-only** Keycloak-admin realm / client / access_tier-mapper / persona seeding — needs direct Keycloak admin access the ingress doesn't expose; production shared_idp provisioning belongs in the auth chart's init-Job pipeline, ENG-8573). Secrets from env vars, never argv.
