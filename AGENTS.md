...
- **`kamiwaza-federation` CLI (ENG-8213):** an operator/test utility (`kamiwaza_sdk.seeding.federation`; console script `kamiwaza-federation`) for shared identity-provider setup and ReBAC access seeding. Groups: `access` (ReBAC grants), `fed` (pair / status / allow-user / unpair), `dataset` / `gate` / `attr` (attribute-gated retrieval setup), and `idp` (**DEV/TEST-only** Keycloak-admin realm / client / attribute-mapper / user seeding — needs direct Keycloak admin access the ingress does not expose; production shared identity-provider provisioning belongs in the auth chart's init-Job pipeline, ENG-8573). Secrets come from environment variables, never arguments.
