 ...
 * spoof (e.g. `x-user-workroom-role: admin`) would slip through
 * unchanged and make local auth tests pass for permissions the user
 * doesn't actually have. Sourced from the shared
 * `ENVELOPE_AUTH_HEADERS` constant so
...
export function _buildBridgedHeaders(
    ...
    // values to the backend, making local auth tests pass in ways
    // production wouldn't — in production the
    ...
    // ``x-user-workroom-role: admin`` reached the backend untouched. The
