...
* When the gate is set, all forwarded-auth envelope headers on the
  ...
  values are injected — defends against client-supplied spoofs of fields the
  bridge does not populate, such as `x-user-workroom-role`. Round-13 review
  (codex P2): the prior
...
* `Identity` gains 2 missing fields to match the Python contract:
  `workroomRole` and `requestId`. Existing callers reading the prior shape
  still type-check because TypeScript widens null-allowed fields.
