...
describe("_buildBridgedHeaders", () => {
    ...
    it("preserves inbound Authorization but still sanitizes the envelope", () => {
        ...
        // ``x-user-roles: admin,owner`` and reach the backend with
    ...
    it("clears spoofed envelope headers before bridging (round-6 codex P2)", () => {
        ...
        // envelope headers (e.g. `x-user-roles: admin,owner`,
        // `x-user-workroom-role: admin`) must not have those spoofed
        // values forwarded to the backend.
